...
/** @type {function(string,string|undefined?):AttValue} */
const COORD_PARSER = (value, elName) =>
  elName === 'text' || elName === 'tspan'
    ? new ListOfLengthPercentageAttValue(value)
    : new LengthPercentageAttValue(value);
/** @type {function(string,string|undefined?):AttValue} */
const DIM_PARSER = (value, elName) =>
  elName === 'rect'
...
/** @type {Map<string,function(string,string|undefined?,boolean?):AttValue>} */
...
export function parseNonStyleAttr(local, value, elName, isImportant = false) {
  ...
  return parse
    ? parse(value, elName, isImportant)
