...
/**
 * @param {import('./types.js').CSSPropertyValue} propValue
 * @returns {string}
 */
export function cssToString(propValue) {
  if (!propValue.value) {
    if (
      propValue.parsedValue === undefined ||
      propValue.parsedValue.value === null
    ) {
      throw new Error();
    }
    switch (propValue.parsedValue.type) {
      case 'transform':
        propValue.value = cssStringifyTransform(propValue.parsedValue.value);
        break;
      default:
        throw new Error();
    }
