import { ClassAttValue } from '../attrs/classAttValue.js';
import { updateReferencedDeclarationIds } from '../tools-ast.js';
 ...
 * @typedef{{type:'PseudoClassSelector',name:string,args:string|undefined}} PseudoClassSelector
 * @typedef{{type:'PseudoElementSelector',name:string,args:string|undefined}} PseudoElementSelector
...
export class CSSRule {
  ...
  addReferencedIds(referencedIds) {
    ...
    for (const propValue of this.#declarations.values()) {
      const id = propValue.getReferencedID();
      if (id) {
        addReference(id, this);
      }
    }
  ...
  getDeclarationString() {
    ...
    for (const [p, v] of this.#declarations.entries()) {
      ...
      const value = v.toStyleElementString();
      if (v.isImportant()) {
        s += `${p}:${value} !important`;
      } else {
        s += `${p}:${value}`;
    ...
    switch (simpleSelectors[0].type) {
      case 'ClassSelector':
        return (element) => {
          ...
          return cv
            ? cv.getClassNames().includes(simpleSelectors[0].name)
            : false;
        };
      case 'IdSelector':
        return (element) =>
          element.svgAtts.get('id')?.toString() === simpleSelectors[0].name;
      case 'TypeSelector':
        return (element) => element.local === simpleSelectors[0].name;
    }
  }
   ...
   */
  removeDeclaration(propName) {
    this.#declarations.delete(propName);
  }
  ...
  updateClassNames(renameMap) {
    this.#selector.updateClassNames(renameMap);
  }
...
export class CSSSelector {
  ...
  getFeatures() {
    ...
    const features = new Set();
    if (this.#selectorSequences.length > 1) {
      features.add('combinators');
    }
  ...
  updateClassNames(renameMap) {
    for (const selectorSequence of this.#selectorSequences) {
      selectorSequence.updateClassNames(renameMap);
    }
    this.#str = this.#generateSelectorString();
    if (this.hasPseudos()) {
      this.#strWithoutPseudos = this.#generateSelectorString(false);
  ...
  addFeatures(features) {
    for (const selector of this.#simpleSelectors)
      switch (selector.type) {
        case 'ClassSelector':
          features.add('class-selectors');
          break;
        case 'IdSelector':
          features.add('id-selectors');
          break;
        case 'TypeSelector':
          features.add('type-selectors');
          break;
  ...
  getString(includePseudos) {
    ...
    for (const selector of this.#simpleSelectors) {
      switch (selector.type) {
        ...
        case 'PseudoElementSelector':
          if (includePseudos) {
            s += `${selector.type === 'PseudoClassSelector' ? ':' : '::'}${selector.name}${selector.args ? `(${selector.args})` : ''}`;
          }
          break;
        case 'TypeSelector':
          s += selector.name;
  ...
  updateClassNames(renameMap) {
    for (const selector of this.#simpleSelectors) {
      switch (selector.type) {
        case 'ClassSelector':
          {
            const newId = renameMap.get(selector.name);
            if (newId) {
              selector.name = newId;
            }
          }
