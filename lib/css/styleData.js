...
export class StyleData {
   ...
   * @param {Map<string,import('../types.js').CSSPropertyValue>} [declarations]
  ...
  computeOwnStyle(element, declarations) {
    ...
    for (const rule of this.getSortedRules()) {
      if (rule.matches(element)) {
        ...
        for (const [name, value] of rule.getDeclarations().entries()) {
          if (isDynamic) {
  ...
  /**
   * @param {Map<string,import('../types.js').SVGAttValue|null>} currentStyles
   * @param {Map<string,import('../types.js').SVGAttValue|null>} parentStyles
   */
