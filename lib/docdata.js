...
export class StyleData {
   ...
   * @returns {Map<string,import('../lib/types.js').SVGAttValue|null>}
   */
      ...
      if (rule.matches(element)) {
        ...
        rule.getDeclarations().forEach((value, name) => {
          if (isDynamic) {
            ...
            const hasVars = VAR_REGEXP.test(cssPropToString(value));
            if (hasVars) {
              computedStyles.set(name, null);
              ...
              computedStyles.set(name, cssPropToString(value));
              if (value.important) {
  ...
  /**
   * @param {{element:XastParent,styles?:Map<string,import('../lib/types.js').SVGAttValue|null>}[]} parentInfo
   * @returns {Map<string,import('../lib/types.js').SVGAttValue|null>}
   */
  ...
  getIdsReferencedByProperties() {
    ...
    for (const rule of this.#sortedRules) {
      ...
      properties.forEach((value) =>
        ids.push(...findReferences('style', cssPropToString(value))),
      );
    }
    return ids;
  }
  ...
  visit(root, {
    element: {
      enter: (node, parentList) => {
        parents.set(node, parentList[parentList.length - 1].element);
