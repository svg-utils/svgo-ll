...
import SAX from 'sax';
import { createRoot } from './xast.js';
...
export function parseSvg(data, from) {
  ...
  sax.onopentag = (data) => {
    ...
    const attributes = {};
    /** @type {import('./types.js').XastAttOther[]} */
    const otherAttrs = [];
    ...
    for (const [name, attr] of Object.entries(data.attributes)) {
      if (!isSVGElement) {
        otherAttrs.push({
          local: attr.local,
          prefix: attr.prefix === '' ? undefined : attr.prefix,
          value: attr.value,
        });
      }
      let attName = name;
    ...
    const element = {
      ...
      local: data.local,
      prefix: data.prefix,
      uri: isSVGElement ? undefined : data.uri,
      otherAtts: otherAttrs.length === 0 ? undefined : otherAttrs,
      attributes: attributes,
      children: [],
      isSelfClosing: data.isSelfClosing,
    };
