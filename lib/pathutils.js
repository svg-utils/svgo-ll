...
import { ExactNum } from './utils/exactNum.js';
...
export function parsePathCommands(path, maxCommands = Number.MAX_SAFE_INTEGER) {
  ...
  function addCurrentCommand(nextCommand) {
    addArg();
    makeCommand(commands, currentCommand, args);
  ...
  for (let index = 0; index < path.length; index++) {
    ...
    switch (ct) {
      ...
      case ctDot:
        ...
        // https://svgwg.org/svg2-draft/paths.html#PathDataBNF says "." must always be followed by a digit; browsers stop parsing the path
        // if this is not so.
        if (!isDigit(path[index + 1])) {
          throw new PathParseError('"." must always be followed by digit');
        }
      ...
      case ctComma:
        ...
        break;
      default:
        throw new PathParseError(`unexpected character in path: "${c}"`);
    }
  }
...
export class PathParseError extends SVGOError {
  /**
   * @param {string} msg
   */
  constructor(msg) {
 ...
 * @returns {void}
 */
function makeCommand(commands, commandCode, args) {
  if (commandCode === '') {
    return;
  }
  switch (commandCode) {
