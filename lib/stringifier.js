...
function stringifyElement(element, config, state) {
  ...
  if (children === '') {
    // There are no children. Don't self-close tag if in a foreignObject and the tag was not originally self-closed.
    if (state.foreignLevel > 0 && element.isSelfClosing === false) {
      s += '></' + elName + '>';
    } else {
      s += '/>';
    }
