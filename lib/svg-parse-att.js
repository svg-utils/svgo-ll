  ...
  function stringify(transform) {
    switch (transform.name) {
      ...
      case 'rotate':
        ...
        return (
          stringifyPropTranslate(transform.tx, transform.ty) +
          `rotate(${stringifyPropAngle(transform.a)})` +
          stringifyPropTranslate(
            { n: transform.tx.n.negate(), unit: transform.tx.unit },
            { n: transform.ty.n.negate(), unit: transform.ty.unit },
          )
        );
      case 'scale':
        if (
...
function stringifyPropLength(len) {
  return `${len.n.getMinifiedString()}${len.unit}`;
}
