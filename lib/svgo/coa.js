...
import { optimizeResolved, resolvePlugins } from '../svgo.js';
...
async function action(args, opts, command) {
  ...
  if (
    ...
    var matches = String(PKG.engines.node).match(/\d*(\.\d+)*/);
    if (!matches || matches.length === 0) {
      throw new Error();
    }
    var nodeVersion = matches[0];
  ...
  if (opts.plugins) {
    config.pluginNames = opts.plugins;
  }
  if (opts.enable) {
  ...
  if (input.length !== 0) {
    ...
    if (input[0] === '-') {
      ...
      await Promise.all(
        input.map((file, n) =>
          optimizeFile(config, resolvedPlugins, file, output[n]),
        ),
      );
    }
 ...
 * @param {import('../svgo.js').CustomPlugin[]} resolvedPlugins
...
function showAvailablePlugins() {
  ...
  const plugins = [];
  builtinPlugins.forEach((value) => plugins.push(value));
  const list = plugins
