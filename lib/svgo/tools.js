...
import { getStyleDeclarations } from '../css.js';
...
export function getReferencedIds(element) {
  function addStyleURLs() {
    ...
    for (const decl of styles.values()) {
      addURL(RE_ATT_URL, 2, 'style', decl.value);
    }
  }
  ...
  for (const [name, value] of Object.entries(element.attributes)) {
    switch (name) {
      case 'href':
      case 'xlink:href':
        addURL(RE_HREF_URL, 1, name, value);
        break;
      case 'style':
        addStyleURLs();
        break;
      default:
        if (referencesProps.has(name)) {
          addURL(RE_ATT_URL, 2, name, value);
        }
        break;
    }
  }
