...
export function addReferencedIdsInStyleAttribute(refs, properties) {
  for (const v of properties) {
    const attValue = /** @type {import('../types/types.js').StyleAttValue} */ (
      v
    );
    for (const propValue of attValue.values()) {
    ...
    case 'style':
      {
        ...
        const att = element.svgAtts.getAtt(attName);
        for (const [propName, v] of att.entries()) {
          ...
          updateReferencedAtt(att, propName, newId);
        }
      }
      break;
    default:
      updateReferencedAtt(
        element.svgAtts,
        attName,
        getNewId(element.svgAtts.getAtt(attName), idMap),
      );
...
/**
 * @param {import('../types/types.js').SvgAttMap} attMap
 * @param {string} attName
 * @param {string} newId
 */
function updateReferencedAtt(attMap, attName, newId) {
  const newURL = '#' + newId;
  switch (attName) {
    case 'clip-path':
      attMap.set(
