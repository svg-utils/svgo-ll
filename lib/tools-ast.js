 ...
 * @returns {string|undefined}
 */
export function getParentName(element) {
...
/**
 * @param {import('./types.js').XastElement} ancestor
 * @param {import('./types.js').XastElement} descendant
 * @returns {boolean}
 */
export function isDescendantOf(ancestor, descendant) {
  while (true) {
    if (descendant.parentNode === ancestor) {
      return true;
    }
    if (descendant.parentNode.type === 'root') {
      return false;
    }
