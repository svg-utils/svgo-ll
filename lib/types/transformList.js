...
export class TransformList extends DataType {
  ...
  /**
   * @param {{x:ExactNum,y:ExactNum}} coords
   * @returns {{x:ExactNum,y:ExactNum}|undefined}
   */
  transformCoords(coords) {
    /** @type {{x:ExactNum,y:ExactNum}|undefined} */
    let result = coords;
    for (const t of this.#transforms) {
      result = transformCoords(t, result);
      if (result === undefined) {
        return;
      }
    }
 ...
 * @returns {{x:ExactNum,y:ExactNum}|undefined}
 */
function transformCoords(t, coords) {
  switch (t.name) {
    case 'scale': {
      const newX = t.sx.mul(coords.x);
      const newY = t.sy.mul(coords.y);
    ...
    case 'translate': {
      ...
      const newX = t.x.n.add(coords.x);
      const newY = t.y.n.add(coords.y);
      if (newX === undefined || newY === undefined) {
        return;
      }
