...
export const LINEAR_GRADIENT_ATTS = new Set(
  ARR_LINEAR_BB_ATTS.concat(['spreadMethod']),
);
export const RADIAL_GRADIENT_ATTS = new Set(
 ...
 */
export function removeUnusedGradientAtts(
  gradient,
...
function isAlwaysOverridden(
  ...
  if (referencingGradients.some((ref) => !GRADIENT_NAMES.has(ref.local))) {
    return false;
  }
